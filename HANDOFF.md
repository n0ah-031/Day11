...
| 상태 | **로그인 → 업로드 → 검토 → 취합 → 다운로드 → 이력 전 구간 동작.** F2 취합 + F4-1 인증 + F4-2 이력 + F4-3 Admin 콘솔 구현, Supabase 영속화 연결. **실제 업무 파일 8개로 검증** 완료. **F3 hwpx 병합은 엔진·CLI 완료(SDK 전환 게이트 판정: 전환 불필요), 화면·API 미착수** |
...
앞의 세 개는 8개·7개·3개 시나리오가 전부 통과해야 정상이며 **네트워크·API 키 없이** 돌아갑니다(AI 계층은 스텁, 인증은 `AUTH_DISABLED=1`로 끔).
...
| hwpx 병합 화면·API(F3-1·2·4·5·6) | 엔진은 됐다(§4). 남은 것은 업로드 화면(화면 8)·드래그 순서 지정·API·비동기 잡·임시 파일 정리다 |
...
8개·7개·3개·11개 시나리오가 전부 통과해야 정상입니다. 앞의 셋은 네트워크 없이 돌고, `test_auth.py`는 실제 Supabase가 필요합니다.

### 큰 것 — PRD 우선순위 순

1. **hwpx 병합 화면·API(F3)** — 우선순위 ②. **엔진과 SDK 전환 게이트 판정은 끝났습니다**(§4 — 전환 불필요). 남은 것은 F3-1(다중 업로드)·F3-2(드래그로 순서 지정)·F3-4(비동기 잡·진행률)·F3-5(임시 파일 정리)·F3-6(용량 제한)과 화면 8입니다. F2의 `_start_job` 폴링과 업로드 상수를 그대로 재사용하면 됩니다. 엔진은 실제 파일로 화면까지 확인했으니(§4) 바로 붙이면 됩니다
   - 다른 계열 hwpx(머리말·꼬리말, 여러 구역, 도형·차트·메모)로 한 번 더 돌려보는 것도 여기에 붙습니다. 새 양식을 만나면 §4처럼 원본 구조를 먼저 덤프하세요
...
| hwpx 병합(F3) | 엔진·CLI 완료(SDK 전환 불필요 판정, 뷰어 렌더까지 확인). 화면·API·비동기 잡 미착수 |
