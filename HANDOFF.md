...
앞의 두 개는 8개·6개 시나리오가 전부 통과해야 정상이며 **네트워크·API 키 없이** 돌아갑니다(AI 계층은 스텁, 인증은 `AUTH_DISABLED=1`로 끔).
...
**남은 문제 — `부서` 컬럼이 파일명 전체입니다.** 엔진은 부서명을 파일명(확장자 제외)에서 가져오는데(§9), 실제 파일명이 `(수원사업소)2025년도 집중안전점검…`, `…리스트(대구)`, `…리스트(양식)_판교지사`처럼 부서명 위치가 파일마다 다릅니다. 규칙으로 뽑아내기 어렵습니다. 데이터 안에 이미 `지사` 컬럼이 있어 실무상 손실은 없지만, 결과의 `부서` 컬럼은 쓸 수 없습니다. 파일명을 `부서명.xlsx`로 바꿔 올리면 해결됩니다.
