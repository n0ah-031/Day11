...
| 쪽·글자·구역 | `1/12쪽 · 6674글자 · 1/3구역` — 12쪽은 4+4+4, 6674자는 구조 검증값과 정확히 일치 |
| 해석 불가 참조 | 0건 |
...
**미검증(남음)**: 5–12쪽(2·3번째 구역)과 사진의 화면 렌더는 눈으로 확인하지 못했습니다 — 뷰어가 합성 키 이벤트를 받지 않아 페이지를 넘길 수 없었습니다. 사람이 스크롤해 확인해야 합니다. 그리고 이 판정은 **감사 처분요구서 한 계열**에 대한 것입니다. 머리말·꼬리말이 있는 문서, 여러 구역짜리 문서, 개체(도형·차트)·메모·글맵시가 든 문서, hwpx 15개 규모는 아직 안 돌려봤습니다.
...
1. **hwpx 병합 화면·API(F3)** — 우선순위 ②. **엔진과 SDK 전환 게이트 판정은 끝났습니다**(§4 — 전환 불필요). 남은 것은 F3-1(다중 업로드)·F3-2(드래그로 순서 지정)·F3-4(비동기 잡·진행률)·F3-5(임시 파일 정리)·F3-6(용량 제한)과 화면 8입니다. F2의 `_start_job` 폴링과 업로드 상수를 그대로 재사용하면 됩니다. 착수 전에 §4 '미검증(남음)'의 눈 확인 1건을 먼저 처리하세요 — 사람이 뷰어에서 5–12쪽을 스크롤해 보면 끝납니다
...
| hwpx 병합(F3) | 엔진·CLI 완료(SDK 전환 불필요 판정). 화면·API·비동기 잡 미착수. 5–12쪽 눈 확인 1건 남음 |
