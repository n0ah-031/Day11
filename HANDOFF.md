...
| 상태 | **로그인 → 업로드 → 검토 → 취합 → 다운로드 → 이력 전 구간 동작.** F2 취합 + F4-1 인증 + F4-2 이력 + F4-3 Admin 콘솔 구현, Supabase 영속화 연결. **실제 업무 파일 8개로 검증** 완료. **F3 hwpx 병합 전 구간 동작**(업로드 → 드래그 순서 지정 → 병합 → 다운로드 → 이력, SDK 전환 게이트 판정: 전환 불필요) |
...
앞의 세 개는 8개·10개·3개 시나리오가 전부 통과해야 정상이며 **네트워크·API 키 없이** 돌아갑니다(AI 계층은 스텁, 인증은 `AUTH_DISABLED=1`로 끔).
...
| [test_server.py](test_server.py) | API 10개 시나리오(엑셀 7개 + 한글 병합 end-to-end·거부·임시 폴더 정리) |
...
1. **AI 양식 생성(F1)** — 미착수, 우선순위 ③. 화면 3만 정적으로 있고 AI 연동이 없습니다. F3까지 끝났으니 이제 여기가 가장 큰 미착수 항목입니다
2. **기존 양식 인지(F6)** — 미착수, 우선순위 ④. F1 인프라를 확장 재사용하는 설계라 F1 뒤에 붙는 것이 맞습니다

### 작은 것 — 언제든 가능

4. **다른 종류의 업무 양식으로 구조 인식 확인.** 엑셀뿐 아니라 **hwpx도 포함**입니다 — 머리말·꼬리말이 있는 문서, 원래 여러 구역인 문서, 도형·차트·메모가 든 문서는 아직 안 돌려봤습니다(§4). 지사별 자체점검 리스트 8개는 검증했지만(§4), 양식이 다르면 또 깨질 수 있습니다. 실제 파일에서 가장 깨지기 쉬운 부분이고, 새 양식을 만나면 §4의 방식대로 원본 구조를 먼저 덤프해 엔진 판정과 대조하세요
...
8. **PRD v2.0 정리.** 이번 회차에 넣은 것 중 PRD에 안 올라간 요구사항이 있습니다 — **부서명 지정**(F2-13·F2-14 옆에 나란히 놓일 항목). §2 구현 완료 표를 PRD 요구사항으로 승격하세요. 여기에 **결정 로그 #4(hwpx 기술 스택) 갱신**을 함께 넣으세요 — M1 게이트 판정이 '전환 불필요'로 나왔습니다(§4)
