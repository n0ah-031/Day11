...
def test_format_consensus(tmp: Path):
    ...
    # 승자가 값의 크기가 아니라 입력 순서를 따른다는 것은 양방향으로만 보인다 —
    # 한쪽만 보면 '작은 값이 이긴다'는 구현도 똑같이 통과한다
    assert xf.consensus([caps[0], caps[3]]).widths["지사"] == 16.2
    assert xf.consensus([caps[3], caps[0]]).widths["지사"] == 38.8
...
def test_format_apply(tmp: Path):
    ...
    # print_area는 openpyxl이 "'시트명'!$A$1:$E$6" 형태로 정규화해 돌려준다
    assert ws.auto_filter.ref == "A4:E6"
    assert ws.print_area.endswith("!$A$1:$E$6")
