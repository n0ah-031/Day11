...
- Produces:
  - `open_sheet(ws, tpl: FormatTemplate, out_headers: list[str]) -> int` — 제목 블록과 헤더 **값**을 쓰고 데이터가 시작될 행 번호를 돌려준다
...
def test_synthesis_inherits_format(tmp: Path):
    ...
    # 부서=A열, 온도차=E열, 데이터는 5행부터. A5가 나온다는 것이 곧 제목 블록이
    # 복원되어 헤더가 4행으로 갔다는 증거다(헤더가 1행이면 A2가 된다)
...
def test_mode_a_own_format(tmp: Path):
    ...
    # 주의: sheet.images의 실제 스키마는 aggregate._read_images가 만든다 —
    # from·to는 1-base (행, 열), marker_from은 0-base (열, 행, 열오프셋, 행오프셋),
    # ext는 EMU가 아니라 파일 확장자 문자열이다
    buf = io.BytesIO()
